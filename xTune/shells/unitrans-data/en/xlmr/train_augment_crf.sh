...
OUTPUT_DIR=/path/to/NER/unitrans-data/xlmr_augment_v1/model/augment_v1_crf/iter1/xlmr-bsz${BATCH_SIZE}-maxlen${MAX_LENGTH}-lr${LR}-epoch${EPOCH}-seed${SEED}-pooling/
mkdir -p $OUTPUT_DIR
python src/run_tag.py --model_type xlmr --model_name_or_path $PRETRAINED_ENCODER_PATH \
      --do_train --do_eval --do_predict --do_predict_dev --predict_langs $PREDICT_LANGS --use_crf \
      ...
      --output_dir $OUTPUT_DIR --overwrite_output_dir --evaluate_during_training --logging_steps 100 --use_pooling_strategy \
      --evaluate_steps $EVALUATE_STEPS --seed $SEED --warmup_steps -1 \
      --save_only_best_checkpoint --eval_all_checkpoints --eval_patience -1 --fp16 --fp16_opt_level O2 --hidden_dropout_prob 0.1 --original_loss | tee -a $OUTPUT_DIR/log.txt \
