DATA_DIR=/path/to/NER/xtreme_bert_UniTrans/panx_processed_maxlen128/
...
OUTPUT_DIR=/path/to/NER/xtreme_bert_UniTrans/model/bert-bsz${BATCH_SIZE}-maxlen${MAX_LENGTH}-lr${LR}-epoch${EPOCH}/
mkdir -p $OUTPUT_DIR
python src/run_tag.py --model_type bert --model_name_or_path $PRETRAINED_ENCODER_PATH \
      ...
      --per_gpu_train_batch_size $BATCH_SIZE --gradient_accumulation_steps $GRAD_ACC --per_gpu_eval_batch_size 384 \
