DATA_DIR=/path/to/NER/xtreme_bert_v1/panx_processed_maxlen128/
...
if [ $PRETRAINED_ENCODER == "bert-large-multilingual-cased" ]; then
    BATCH_SIZE=32
    GRAD_ACC=1
    LR=7e-6
else
...
OUTPUT_DIR=/path/to/NER/xtreme_bert_v1/model/augment_v1/iter1/bert-bsz${BATCH_SIZE}-maxlen${MAX_LENGTH}-lr${LR}-epoch${EPOCH}/
mkdir -p $OUTPUT_DIR
python src/run_tag.py --model_type bert --model_name_or_path $PRETRAINED_ENCODER_PATH \
