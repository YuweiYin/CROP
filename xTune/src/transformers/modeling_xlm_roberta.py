...
class XLMRobertaForTokenClassificationPoolingStable(BertPreTrainedModel):
    ...
    def __init__(self, config, noised_data_generator=None, use_pooling_strategy=False, use_crf=False):
        super().__init__(config)
        self.num_labels = config.num_labels
    ...
    Examples::
        ...
        if labels is not None:
            ...
            if self.training:
                ...
                if self.original_loss:
                    if self.use_crf:
                        tag_masks = (labels != CrossEntropyLoss().ignore_index)
                        ...
                        for b in range(labels.size(0)):
                            ...
                            new_labels.append(labels[b][tag_mask])
                        new_valid_labels = new_labels.copy()
                        for b in range(labels.size(0)):
                            new_emissions[b] = torch.cat([new_emissions[b]] + [new_emissions[b][-1:]] * (seq_len - new_emissions[b].size(0)))
                            new_valid_labels[b] = torch.cat([new_labels[b], labels.new(seq_len - new_labels[b].size(0)).fill_(0)])
                            new_labels[b] = torch.cat([new_labels[b], labels.new(seq_len - new_labels[b].size(0)).fill_(CrossEntropyLoss().ignore_index)])
                        new_emissions = torch.stack(new_emissions, dim=0)
                        new_labels = torch.stack(new_labels, dim=0)
                        new_valid_labels = torch.stack(new_valid_labels, dim=0)
                        crf_mask = new_labels != CrossEntropyLoss().ignore_index
                        crf_loss = self.crf(new_emissions, new_valid_labels, mask=crf_mask, reduction="token_mean")
                        loss = -crf_loss
                    original_loss = loss
                else:
                    original_loss = loss.data.new([0.0])
