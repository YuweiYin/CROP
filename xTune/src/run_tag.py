...
def predict(args, model, tokenizer, labels, pad_token_label_id, mode, prefix="", lang="en", print_result=True):
    ...
    for batch in tqdm(eval_dataloader, desc="Evaluating"):
        ...
        nb_eval_steps += 1
        if model.use_crf:
            emissions = torch.softmax(logits, dim=-1)
            crf_preds.append(model.crf.decode(emissions).detach().cpu().numpy())
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, prefix=""):
    ...
    for split, lang in eval_datasets:
        ...
        for batch in eval_dataloader:
            ...
            if model.use_crf:
                emissions = torch.softmax(logits, dim=-1)
                tag_masks = (out_label_ids != CrossEntropyLoss().ignore_index)
                new_emissions = []
                new_labels = []
                bsz, seq_len = emissions.size(0), emissions.size(1)
                for b in range(bsz):
                    tag_mask = tag_masks[b]
                    new_emissions.append(emissions[b][tag_mask])
                    new_labels.append(out_label_ids[b][tag_mask])
                new_valid_labels = new_labels.copy()
                for b in range(bsz):
                    new_emissions[b] = torch.cat(
                        [new_emissions[b]] + [new_emissions[b][-1:]] * (seq_len - new_emissions[b].size(0)))
                    new_valid_labels[b] = torch.cat(
                        [batch[0].new(new_labels[b]), batch[0].new(seq_len - new_emissions[b].size(0)).fill_(0)])
                new_emissions = torch.stack(new_emissions, dim=0)
                crf_mask = new_labels != CrossEntropyLoss().ignore_index
                crf_pred = model.crf.decode(new_emissions, mask=crf_mask).detach().cpu().numpy()
...
def main():
    ...
    parser.add_argument("--use_crf", action="store_true", help="Whether to use crf layer.")
    ...
    if args.do_predict and args.local_rank in [-1, 0]:
        ...
        with open(output_test_results_file, "a") as result_writer:
            for lang in args.predict_langs.split(','):
                if not os.path.exists(os.path.join(args.data_dir, lang, 'test.xlmr')):
                ...
                if args.test_output_file != "":
                    output_test_predictions_file = args.test_output_file
                    infile = os.path.join(args.data_dir, lang, "test.xlmr")
                    output_word_prediction = True
                else:
                    output_word_prediction = False
                ...
                save_predictions(args, predictions, output_test_predictions_file, infile, idxfile,
                                 output_word_prediction=output_word_prediction)
...
def save_predictions(args, predictions, output_file, text_file, idx_file, output_word_prediction=False):
    ...
    logger.info(f"Saving Prediction Results to {output_file}")
    # Sanity check on the predictions
    with open(output_file, "w") as writer:
        example_id = 0
        prev_id = int(index[0])
        for i, (line, idx) in enumerate(zip(text, index)):
            if line == "" or line == "\n":
                example_id += 1
            else:
                cur_id = int(idx)
