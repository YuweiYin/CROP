...
class AlignDataset:
    def __init__(self, subword_align_dataset, phrase_align_dataset=None):
        self.subword_align_dataset = subword_align_dataset
        self.phrase_align_dataset = phrase_align_dataset
    ...
    def __getitem__(self, index):
        return self.subword_align_dataset[index]  if self.subword_align_dataset is not None else None, self.phrase_align_dataset[index] if self.phrase_align_dataset is not None else None

    def __len__(self):
        if self.subword_align_dataset is not None:
            return len(self.subword_align_dataset)
        else:
            return len(self.phrase_align_dataset)
...
class MultilingualDatasetManager(object):
    ...
    def move_align_idx(self, align_dataset):
        if self.args.langtoks['main'][0] is not None:  # start from the index 1
            for align in align_dataset:
                if len(align) > 0:
                    align[:, 0] += 1
        if self.args.langtoks['main'][1] is not None:  # start from the index 1
            for align in align_dataset:
                if len(align) > 0:
                    align[:, 1] += 1
    ...
    def load_lang_dataset(
        ...
        align_dataset = None
        if load_alignments:
            if os.path.exists(os.path.join(data_path, "{}.align.{}-{}.npy".format(split, src, tgt))):
                align_path = os.path.join(
                    data_path, "{}.align.{}-{}.npy".format(split, src, tgt)
                )
            elif os.path.exists(os.path.join(data_path, "{}.align.{}-{}.npy".format(split, tgt, src))):
                align_path = os.path.join(
                    data_path, "{}.align.{}-{}.npy".format(split, tgt, src)
                )
            else:
                align_path = ""
            if os.path.exists(align_path):
                if self.args.subword_prob > 0:
                    subword_align_dataset = list(np.load(align_path, allow_pickle=True))
                    subword_align_dataset = self.move_align_idx(subword_align_dataset)
                    logger.info("Loading subword-level align dataset: {} {} examples".format(align_path, len(subword_align_dataset)))
                else:
                    subword_align_dataset = None
                phrase_align_path = align_path.replace("align", "align.phrase")
                if os.path.exists(phrase_align_path) and self.args.subword_prob < 1.0: #
                    phrase_align_dataset = list(np.load(phrase_align_path, allow_pickle=True))
                    phrase_align_dataset = self.move_align_idx(phrase_align_dataset)
                    logger.info("Loading phrase-level align dataset: {} {} examples".format(align_path, len(phrase_align_dataset)))
                else:
                    phrase_align_dataset = None
                align_dataset = AlignDataset(subword_align_dataset, phrase_align_dataset)
    ...
    def load_langpair_dataset(
        ...
        if langpairs_sharing_datasets is not None:
            ...
            elif "bt" in data_path: # distinguish the x2e, e2x, x2y, y2x bt
                src_dataset = "NotInCache"
                tgt_dataset = "NotInCache"
                align_dataset = "NotInCache"
            else:
                src_dataset = langpairs_sharing_datasets.get(
            ...
            if langpairs_sharing_datasets is not None:
                if "parallel_bt" in data_path:
                    ...
                    if align_dataset is None:
                        ...
                        reversed_subword_align_dataset, reversed_phrase_align_dataset = align_dataset.reversed_align_dataset()
                        reversed_align_dataset = AlignDataset(reversed_subword_align_dataset, reversed_phrase_align_dataset)
                        langpairs_sharing_datasets[
                            (data_path, split, norm_direction, tgt, src)
                        ] = reversed_align_dataset
        else:
    ...
    def get_train_sampling_ratios(
        ...
        if getattr(self.args, 'data_param_list_sampling_ratios', None) is None:
            ...
            #logger.info("Equal Training Weight for All Data Param List main, bt, parallel_bt")
        else:
            assert ["main"] + list(self.args.extra_lang_pairs.keys()) == list(self.args.data_param_list_sampling_ratios.keys())
            logger.info(self.args.data_param_list_sampling_ratios)
            data_param_list_weights = list(self.args.data_param_list_sampling_ratios.values())
            #assert sum(data_param_list_weights) == 1.0, "{}".format(sum(data_param_list_weights))
            sample_ratios = [ratio/sum(ratios) * data_param_list_weights[i] for i, ratios in enumerate(sample_ratios) for ratio in ratios] if sample_ratios is not None else None
    ...
    def load_sampled_multi_dataset(
        ...
        if training and split == getattr(self.args, "train_subset", None):
            # sample_ratios = self.get_sampling_ratios(data_param_list, datasets, epoch)
            sample_ratios_list = [self.get_sampling_ratios(data_param_list, datasets, i) for i in range(1000)]
            sample_ratios = sample_ratios_list[epoch]
            if self.args.sampling_method == 'linear' and self.args.virtual_data_size is None:
                self.args.virtual_data_size = sum([len(d) for d in OrderedDict(datasets).values()])
            return SampledMultiDataset(
