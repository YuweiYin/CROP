...
class XLMTDecoderVariantModel(TransformerFromPretrainedInfoXLMModel):
    ...
    def forward(
        ...
        **extra_args
        ...
        # if "src_lang_id" in extra_args:
        #     extra_args["src_lang_id"] = self.get_lang_id(extra_args["src_lang_id"]) - 1
        # if "tgt_lang_id" in extra_args:
        #     extra_args["tgt_lang_id"] = self.get_lang_id(extra_args["tgt_lang_id"]) - 1
...
class XLMTDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False, layer_id=0):
        ...
        if getattr(args, 'freeze_decoder', False):
            for param in self.layers.parameters():
                param.requires_grad = False
            self.start_adapter_layer = getattr(args, "start_adapter_layer", 0)
            for i in range(self.start_adapter_layer, args.decoder_layers):
                for param in self.layers[i].adapter_down_proj:
                    if param is not None:
                        param.requires_grad = True
                for param in self.layers[i].adapter_up_proj:
                    if param is not None:
                        param.requires_grad = True
                for param in self.layers[i].adapter_layer_norm:
                    if param is not None:
                        param.requires_grad = True
                # for param in self.layers[i].adapter_weight:
                #     if param is not None:
                #         param.requires_grad = True
        if getattr(args, 'freeze_embedding', False):
            for param in self.embed_tokens.parameters():
        ...
        #     self.embed_adapter_down_proj = nn.ModuleList([])
        #     self.embed_adapter_up_proj = nn.ModuleList([])
        #     self.embed_adapter_layer_norm = nn.ModuleList([])
        #     self.embed_adapter_weight = nn.ParameterList([])
        #     used_langs = [lang_pair.split("-")[0] for lang_pair in args.lang_pairs.split(',')] + [lang_pair.split("-")[1] for lang_pair in args.lang_pairs.split(',')]
        #     used_langs = set(used_langs)
        #     for i in range(len(args.langs)):
        #         if args.langs[i] in used_langs:
        #             self.embed_adapter_down_proj.append(nn.Linear(self.embed_dim, self.adapter_dim))
        #             self.embed_adapter_up_proj.append(nn.Linear(self.adapter_dim, self.embed_dim))
        #             self.embed_adapter_layer_norm.append(LayerNorm(self.embed_dim))
        #             self.embed_adapter_weight.append(Weight(self.embed_dim))
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False, layer_id=0):
        if args.variant == 'first':
            ...
            layer = XLMTAddFFN(args, no_encoder_attn, layer_id=layer_id)
        elif args.variant == 'gpt-two-attn':
    ...
    def embed_adapter_forward(self, x, lang_id):
        #residual = x
        #if self.normalize_before:
        x = self.embed_adapter_layer_norm[lang_id](x)
        x = self.layers[-1].activation_fn(self.embed_adapter_down_proj[lang_id](x))
        x = self.layers[-1].activation_dropout_module(x)
        x = self.embed_adapter_up_proj[lang_id](x)
        x = self.layers[-1].dropout_module(x)
        x = self.embed_adapter_weight[lang_id].unsqueeze(0).unsqueeze(1) * x
...
class XLMTAddFFN(TransformerDecoderLayer):
    ...
    def __init__(
        ...
        if self.adapter_dim > 0:
            ...
            #self.adapter_weight = nn.ParameterList([])
            self.high_langs = getattr(args, "high_langs", args.langs)
            used_langs = [lang_pair.split("-")[0] for lang_pair in args.lang_pairs.split(',')] + [lang_pair.split("-")[1] for lang_pair in args.lang_pairs.split(',')]
            used_langs = set(used_langs)
            if layer_id >= self.start_adapter_layer:
                for i in range(len(args.langs)):
                    if args.langs[i] in used_langs:
                        ...
                        #self.adapter_weight.append(Weight(self.embed_dim))
                    else:
                        self.adapter_down_proj.append(None)
                        self.adapter_up_proj.append(None)
                        self.adapter_layer_norm.append(None)
                        #self.adapter_weight.append(None)
                        # self.adapter_down_proj.append(self.build_fc1(1, 1, self.quant_noise, self.quant_noise_block_size))
                        # self.adapter_up_proj.append(self.build_fc2(1, 1, self.quant_noise, self.quant_noise_block_size))
                        # self.adapter_layer_norm.append(LayerNorm(1))
                        # self.adapter_weight.append(Weight(1))
        # End #
    ...
    def adapter_forward(self, x, lang_id):
        ...
        x = self.dropout_module(x)
        # x = self.adapter_weight[lang_id].unsqueeze(0).unsqueeze(1) * x
        #x = self.residual_connection(x, residual)
        #if not self.normalize_before:
        #    x = self.adapter_layer_norm[lang_id](x)
        return x
    ...
    def forward(
        ...
        #if self.adapter_dim > 0 and len(getattr(self, "adapter_down_proj", [])) > 0:
        ...
        #Parallel Adapter (Added By Jian Yang)
        if self.adapter_dim > 0 and len(getattr(self, "adapter_down_proj", [])) > 0:
            if isinstance(tgt_lang_id, list) and sum(tgt_lang_id) == tgt_lang_id[0] * len(tgt_lang_id):
                tgt_lang_id = tgt_lang_id[0]
            if isinstance(tgt_lang_id, int) or (tgt_lang_id.sum() == tgt_lang_id[0][0] * tgt_lang_id.size(0)):
...
def base_architecture(args):
    ...
    #
    args.adapter_dim = getattr(args, "adapter_dim", args.decoder_ffn_embed_dim // 4)
