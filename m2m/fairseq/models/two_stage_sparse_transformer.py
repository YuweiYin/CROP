...
class TwoStageSparseDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.end_soft_epoch = getattr(args, "end_soft_epoch", 1)
        ...
        if self.use_adapter:
            ...
            self.gate = nn.Linear(embed_tokens.embedding_dim, len(self.adapter) - 1, bias=True)
        else:
            self.adapter = None
        ...
        if self.training_phase == "phase-2":
    ...
    def swap_adapter_forward(self, x, swap_adapter_prob, hard_or_soft_adapter_prob, soft_weights, tgt_lang_id, epoch):
        lang_similarity = torch.tensor([0]).type_as(x)
        lang_soft_weights = torch.tensor([0]).type_as(x)
        if swap_adapter_prob < self.swap_adapter and self.training:
            x = self.adapter[-1](x)  #universal adapter
        else:
            if self.langs[tgt_lang_id] in self.high_langs:
                if (hard_or_soft_adapter_prob < self.hard_adapter and epoch >= self.start_hard_epoch) or not self.training or epoch >= self.end_soft_epoch:
    ...
    def extract_features_scriptable(
        ...
        if self.use_adapter:
            #x, lang_soft_weights, lang_similarity = self.swap_adapter_forward(x, swap_adapter_prob=swap_adapter_prob, hard_or_soft_adapter_prob=hard_or_soft_adapter_prob, tgt_lang_id=tgt_lang_id, soft_weights=soft_weights, epoch=epoch)
