...
@register_model("transformer_from_pretrained_infoxlm_lang")
class TransformerFromPretrainedInfoXLMLangModel(TransformerModel):
    @staticmethod
    def add_args(parser):
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TransformerDecoderFromPretrainedInfoXLM(args, tgt_dict, embed_tokens)
    ...
    for key in infoxlm_state_dict.keys():
        ...
        if 'lm_head' not in key:
            if 'in_proj_weight' in key:
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'q_proj.weight')] = infoxlm_state_dict[key][:768]
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'k_proj.weight')] = infoxlm_state_dict[key][768:1536]
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'v_proj.weight')] = infoxlm_state_dict[key][1536:]
                if shared_cross_attn:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'q_proj.weight').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][:768]
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'k_proj.weight').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][768:1536]
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_weight', 'v_proj.weight').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][1536:]
            elif 'in_proj_bias' in key:
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'q_proj.bias')] = infoxlm_state_dict[key][:768]
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'k_proj.bias')] = infoxlm_state_dict[key][768:1536]
                state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'v_proj.bias')] = infoxlm_state_dict[key][1536:]
                if shared_cross_attn:
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'q_proj.bias').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][:768]
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'k_proj.bias').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][768:1536]
                    state_dict[key.replace('decoder.sentence_encoder.', '').replace('in_proj_bias', 'v_proj.bias').replace('self_attn', 'encoder_attn')] = infoxlm_state_dict[key][1536:]
...
@register_model_architecture(
    "transformer_from_pretrained_infoxlm_lang", "transformer_from_pretrained_infoxlm_lang"
)
