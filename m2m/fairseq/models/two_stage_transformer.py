...
@register_model("two_stage_transformer")
class TwoStageModel(TransformerFromPretrainedInfoXLMModel):
    @staticmethod
    def add_args(parser):
    ...
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return TwoStageDecoder(args, tgt_dict, embed_tokens)
...
class TwoStageEncoder(TransformerEncoderFromPretrainedInfoXLM):
...
class TwoStageDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        self.swap_adapter = getattr(args, "swap_adapter", -1)
        ...
        if self.use_adapter:
            ...
            self.adapter.extend(
                nn.ModuleList(
                    [
                        AdapterLayer(args)
                        for _ in range(len(self.high_langs) + 1)
                    ])
            )
        else:
            self.adapter = None
    ...
    def swap_adapter_forward(self, x, tgt_lang_id):
        if self.use_adapter:
            if torch.rand((1,)) > self.swap_adapter or not self.training:
    ...
    def extract_features_scriptable(
        ...
        # if self.use_adapter:
        #     x = self.swap_adapter_forward(x, tgt_lang_id)
...
@register_model_architecture(
    "two_stage_transformer", "two_stage_transformer"
)
def base_architecture(args):
    ...
    args.swap_adapter = getattr(args, "swap_adapter", -1)


@register_model_architecture(
    "two_stage_transformer", "two_stage_transformer_large"
)
def base_architecture(args):
    args.encoder_embed_path = getattr(args, "encoder_embed_path", None)
