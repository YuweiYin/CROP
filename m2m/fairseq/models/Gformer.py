...
def CopyDecoderBottom2Top(state_dict, decoder_group_layers, decoder_group_number):
    ...
    for i in range(decoder_group_layers):
        for j in range(1, decoder_group_number):
            state_dict[f'layers.{get_layer(i, j)}.self_attn.k_proj.weight'] = state_dict[f'layers.{i}.self_attn.k_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.k_proj.bias'] = state_dict[f'layers.{i}.self_attn.k_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.v_proj.weight'] = state_dict[f'layers.{i}.self_attn.v_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.v_proj.bias'] = state_dict[f'layers.{i}.self_attn.v_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.q_proj.weight'] = state_dict[f'layers.{i}.self_attn.q_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.q_proj.bias'] = state_dict[f'layers.{i}.self_attn.q_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.out_proj.weight'] = state_dict[f'layers.{i}.self_attn.out_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.self_attn.out_proj.bias'] = state_dict[f'layers.{i}.self_attn.out_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.self_attn_layer_norm.weight'] = state_dict[f'layers.{i}.self_attn_layer_norm.weight']
            state_dict[f'layers.{get_layer(i, j)}.self_attn_layer_norm.bias'] = state_dict[f'layers.{i}.self_attn_layer_norm.bias']

            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.k_proj.weight'] = state_dict[f'layers.{i}.encoder_attn.k_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.k_proj.bias'] = state_dict[f'layers.{i}.encoder_attn.k_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.v_proj.weight'] = state_dict[f'layers.{i}.encoder_attn.v_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.v_proj.bias'] = state_dict[f'layers.{i}.encoder_attn.v_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.q_proj.weight'] = state_dict[f'layers.{i}.encoder_attn.q_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.q_proj.bias'] = state_dict[f'layers.{i}.encoder_attn.q_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.out_proj.weight'] = state_dict[f'layers.{i}.encoder_attn.out_proj.weight']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn.out_proj.bias'] = state_dict[f'layers.{i}.encoder_attn.out_proj.bias']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn_layer_norm.weight'] = state_dict[f'layers.{i}.encoder_attn_layer_norm.weight']
            state_dict[f'layers.{get_layer(i, j)}.encoder_attn_layer_norm.bias'] = state_dict[f'layers.{i}.encoder_attn_layer_norm.bias']
            ...
            state_dict[f'layers.{get_layer(i, j)}.ffn_layer_norm.weight'] = state_dict[f'layers.{i}.ffn_layer_norm.weight']
            state_dict[f'layers.{get_layer(i, j)}.ffn_layer_norm.bias'] = state_dict[f'layers.{i}.ffn_layer_norm.bias']
            state_dict[f'layers.{get_layer(i, j)}.final_layer_norm.weight'] = state_dict[f'layers.{i}.final_layer_norm.weight']
            state_dict[f'layers.{get_layer(i, j)}.final_layer_norm.bias'] = state_dict[f'layers.{i}.final_layer_norm.bias']
    return state_dict
...
class GformerModel(TransformerFromPretrainedInfoXLMModel):
    ...
    def forward(
        ...
        encoder_out = self.encoder(
            src_tokens, src_lengths=src_lengths, return_all_hiddens=True, src_lang_id = src_lang_id, tgt_lang_id=tgt_lang_id
        )
...
class GformerEncoder(TransformerEncoder):
    def __init__(self, args, dictionary, embed_tokens):
        ...
        if hasattr(args, "pretrained_infoxlm_checkpoint") and os.path.exists(args.pretrained_infoxlm_checkpoint):
            ...
            if args.encoder_layers > args.encoder_group_layers and args.encoder_layers % args.encoder_group_layers == 0 and args.recurrent_init:
                infoxlm_loaded_state_dict = CopyEncoderBottom2Top(infoxlm_loaded_state_dict, args.encoder_group_layers, args.encoder_layers//args.encoder_group_layers)
        ...
        if self.encoder_group_type == "sparse":
            ...
            self.encoder_fusion_weight = nn.ParameterList([Weight(self.encoder_group_number) for _ in range(len(self.high_langs + self.low_langs))])
    ...
    def forward(
        ...
        if self.encoder_group_type == "sparse":
            x = torch.stack([encoder_states[i] for i in range(self.encoder_group_layers - 1, len(encoder_states), self.encoder_group_layers)], dim=0)
...
class AdapterLayer(nn.Module):
    ...
    def forward(
        ...
        residual = x
        x = self.adapter_forward_with_residual(x, encoder_out, encoder_padding_mask, incremental_state, prev_self_attn_state, prev_attn_state, self_attn_mask, self_attn_padding_mask, need_attn, need_head_weights)
...
class GformerDecoder(TransformerDecoder):
    def __init__(self, args, dictionary, embed_tokens, no_encoder_attn=False):
        ...
        if hasattr(args, "pretrained_infoxlm_checkpoint") and os.path.exists(args.pretrained_infoxlm_checkpoint):
            if args.variant == 'addffn':
                ...
                if args.decoder_layers > args.decoder_group_layers and args.decoder_layers % args.decoder_group_layers == 0 and args.recurrent_init:
                    infoxlm_loaded_state_dict = CopyDecoderBottom2Top(infoxlm_loaded_state_dict,  args.decoder_group_layers, args.decoder_layers // args.decoder_group_layers)
                    print("Recurrent Initialization !")
        ...
        self.language_sepcific_lr = [float(w) for w in args.language_specific_lr.split(",")] if hasattr(args, "language_specific_lr") else None
    ...
    def extract_features(
        ...
        if self.decoder_group_type == "dense":
            ...
            selected_states = [inner_states[i] for i in range(self.decoder_group_layers - 1, len(inner_states), self.decoder_group_layers)]
            if self.use_adapter:
