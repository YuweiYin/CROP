...
SLOT_PROB=0.85
MAX_SPAN_LENGTH=256
MODEL=/path/to/NER/m2m/model/M2M-102/M2M102-lr_${LR}-SLOT_PROB_${SLOT_PROB}-subword_prob_${SUBWORD_PROB}-MAX_TOKENS_${MAX_TOKENS}-UPDATE_FREQ_${UPDATE_FREQ}-MAX_SLOT_NUM_${MAX_SLOT_NUM}-NODES_${NODES}-GPUS_${GPUS}-SLOT_METHOD-${SLOT_METHOD}/
mkdir -p $MODEL
echo "Start Training ..."
