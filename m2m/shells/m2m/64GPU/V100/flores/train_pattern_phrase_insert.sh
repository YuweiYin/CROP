...
SLOT_METHOD="insert"
SLOT_PROB=0.85
MODEL=/path/to/NER/flores/20M/model/insert/M2M102-lr_${LR}-SLOT_PROB_${SLOT_PROB}-subword_prob_${SUBWORD_PROB}-MAX_TOKENS_${MAX_TOKENS}-UPDATE_FREQ_${UPDATE_FREQ}-MAX_SLOT_NUM_${MAX_SLOT_NUM}-NODES_${NODES}-GPUS_${GPUS}-SLOT_METHOD-${SLOT_METHOD}/
mkdir -p $MODEL
echo "Start Training ..."
if [ ! -f $MODEL/checkpoint_last.pt ]; then  #compatible with preemptible job
    python -m torch.distributed.launch --nproc_per_node=$GPUS --nnodes=$NODES --node_rank=$OMPI_COMM_WORLD_RANK --master_addr=$MASTER_ADDR --master_port=$MASTER_PORT train.py $TEXT \
        ...
        --subword-prob $SUBWORD_PROB --max-slot-num $MAX_SLOT_NUM --slot-prob $SLOT_PROB --slot-method $SLOT_METHOD --virtual-epoch-size 40000000 \
