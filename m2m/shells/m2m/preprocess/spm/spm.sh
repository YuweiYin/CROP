...
M2M_SPM_MODEL=/path/to/NER/PretrainedModels/m2m-100/spm.128k.model
INPUT_DIR=/path/to/NER/m2m/cc_bpe/
RAW_DIR=/path/to/NER/m2m/raw/
SPM_DIR=/path/to/NER/m2m/m2m_bpe/
OUTPUT_DIR=/path/to/NER/m2m/All/
...
if [ $STATE = "ALL" -o $STATE = "FILTER" ]; then 
    ...
    python $FILTER -src $SRC -tgt $TGT -new-src $NEW_SRC -new-tgt $NEW_TGT -length-ratio 1.5 -max-length 250 -max-sentences 100000000000000
fi
