...
INPUT_DIR=/path/to/NER/flores/20M/train/
OUTPUT_DIR=/path/to/NER/flores/20M/align/
ALIGN=./scripts/align.py
MAKE_PRIORS=./scripts/makepriors.py
PREPROCESS_ALIGN=./scripts/pattern/preprocess_alignments.py
STATE="ALL" #ALL CONCAT ALIGN MAKE_PRIORS
mkdir -p $OUTPUT_DIR
lg=$1
...
if [ $STATE = "ALL" -o $STATE = "MAKE_PRIORS" ]; then
