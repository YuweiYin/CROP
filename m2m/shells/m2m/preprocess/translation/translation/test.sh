...
TEXT=/path/to/NER/flores/evaluation/devtest-code/
...
cat $INPUT | python ./fairseq_cli/interactive.py $DATA_BIN \
    ...
    --lang-pairs $LANG_PAIRS --max-len-b 256 --min-len 1 --nbest 1 \
    --source-lang $src --target-lang $tgt \
    --buffer-size 10000 --batch-size $batchsize --beam $beam --lenpen $lenpen --unkpen 10000 \
    --no-progress-bar --fp16 --bpe sentencepiece --sentencepiece-model $SPM_MODEL > $OUTPUT.log
cat $OUTPUT.log | grep -P "^D" | cut -f 3- > $FOUT

BLEU_DIR=/path/to/NER/flores/evaluation/BLEU/
mkdir -p $BLEU_DIR
evaluation_tok="spm"
