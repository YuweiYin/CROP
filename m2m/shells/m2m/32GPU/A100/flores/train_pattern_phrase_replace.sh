...
SLOT_METHOD="replace"
SLOT_PROB=0.85
MODEL=/path/to/NER/flores/20M/model/replace/lr_${LR}-SLOT_PROB_${SLOT_PROB}-subword_prob_${SUBWORD_PROB}-MAX_TOKENS_${MAX_TOKENS}-UPDATE_FREQ_${UPDATE_FREQ}-MAX_SLOT_NUM_${MAX_SLOT_NUM}-NODES_${NODES}-GPUS_${GPUS}-SLOT_METHOD-${SLOT_METHOD}/
mkdir -p $MODEL
echo "Start Training ..."
