...
DATA_BIN=/path/to/NER/flores/20M/data-bin-split20/data-bin0/
INPUT_DIR=/path/to/NER/xtreme_translate_train/translate_train/LABELED_EN/
OUTPUT_DIR=/path/to/NER/xtreme_v1/translate_train/LABELED_X/
mkdir -p $OUTPUT_DIR
src=$1
tgt=$2
beam=$3
...
cat $INPUT | python ./fairseq_cli/interactive.py $DATA_BIN \
    ...
    --buffer-size $BUFFER_SIZE --batch-size $BATCH_SIZE --beam $beam --lenpen $lenpen \
