...
def parse_args():
    ...
    parser.add_argument('--en-input', '-en-input', type=str,
                        default=r'/path/to/NER/xtreme_v1/translation/LABELED_EN/de.txt0000', help='input stream')
    parser.add_argument('--input', '-input', type=str,
                        default=r'/path/to/NER/xtreme_v1/translation/LABELED_X/en0000.2de', help='input stream')
    parser.add_argument('--beam-size', '-beam-size', type=int,
                        default=1, help='beam size')
    parser.add_argument('--ner', '-ner', type=str,
                        default=r'/path/to/NER/xtreme_v1/translation/NER/de/test.xlmr.tsv', help='input stream')
    parser.add_argument('--output', '-output', type=str,
                        default=r'/path/to/NER/xtreme_v1/translation/LABELED_X_NER/train.de.xlmr',
                        help='input stream')
    parser.add_argument('--idx', '-idx', type=str,
                        default=r'/path/to/NER/xtreme_v1/translation/LABELED_X_NER/train.de.xlmr.idx',
                        help='input stream')
...
if __name__ == "__main__":
    ...
    for _ in range(args.beam_size):
        beam_examples = [examples[i] for i in range(args.beam_size, len(examples), args.beam_size)]
            ...
            for i in range(len(beam_ner_words[0])):
                for k in range(args.beam_size):
                    ...
                    if i != len(beam_ner_words[0]) - 1 or k != args.beam_size - 1:
                        ...
                        idx_w.write("\n")
    logger.info(
        f"Successfully Saving to {args.output} | Examples: {len(examples)} -> {len(beam_ner_words) * len(beam_ner_words[0])}")
    logger.info(
        f"Successfully Saving to {args.idx} | Examples: {len(examples)} -> {len(beam_ner_words) * len(beam_ner_words[0])}")
